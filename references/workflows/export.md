...
- 处理仅做旋转纠正、空白边裁切、等比缩放和必要留白；不拉伸、不裁脸、不改学校标识内部构图。
- 仅在用户确认使用后，才把对应 `<img>` 加入简历 HTML；DOCX 导出会从同一 HTML 读取图片。

标准化后必须先查看图片，再查看最终 A4 渲染：不要把“比例正确”当作视觉验收。横向组合标的可见宽度通常应接近 38–40 mm，方形/近方形校徽的可见高度通常应接近 14–16 mm；若显得偏小，先检查并去除素材内部无效留白，再在边界范围内调整。不得用超出边界或拉伸的方式补救。
